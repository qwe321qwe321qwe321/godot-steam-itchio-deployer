...
Build, Steam, and itch.io configuration panels are arranged as three equal horizontal columns inside their own scrolling settings area, and each column can be collapsed independently. Narrow docks can scroll the settings area horizontally without moving the pinned controls. The top-level Build / Deploy Config picker and the Save/Build/Upload action bar are pinned above that scroll area, so they remain visible while editing long platform forms. The Steam and itch.io Resource pickers are grouped at the top of the collapsible Build panel. **Console Result** is a separate collapsible panel below the settings area and expands automatically when a Build, Upload, or Build & Upload workflow starts. If Steam Guard is requested, the Steam panel expands automatically before the temporary code input appears. Each **Save Encrypted Credentials** action sits directly below that platform's secret input. **Save Settings** gains an asterisk and yellow highlight whenever the current non-secret configuration differs from the saved files; credential edits are intentionally tracked separately.
